    ...
    <>
          ...
          {overview.rows.map((r) => {
            ...
            return (
              <tr key={r.sellerId}>
                ...
                <td className="whitespace-nowrap"><b className={r.availableN ? "text-[#16794a]" : "text-[#b3b5bc]"}>{m(r.availableCents, r.currency)}</b><div className="text-[11px] text-[#8a8d96]">{t(`${r.availableN} orders`, `${r.availableN}건`)}</div></td>
      ...
      <Panel title={<>{t("Settlements", "정산서")} <span className="ml-1 text-[#8a8d96]">{total}</span></>} bodyClass="p-0">
        ...
        >
          {rows.map(({ st, seller }) => (
            <tr key={st.id}>
              <td className="whitespace-nowrap text-xs"><Link href={`/admin/settlements/${st.id}`} className="font-semibold text-[#2f4ac2] hover:underline">{formatDate(st.createdAt, lang, true)}</Link></td>
