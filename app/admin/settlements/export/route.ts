...
export async function GET(request: Request) {
  ...
  return csvResponse(`ringo-settlements-${new Date().toISOString().slice(0, 10)}.csv`, [
    ...
    ...rows.map(({ st, x }) => [
      st.id, st.createdAt.toISOString(), x.displayName, x.slug, st.periodStart.toISOString(), st.periodEnd.toISOString(), st.orderCount, st.currency,
      major(st.grossCents), major(st.commissionCents), major(st.netCents), st.status, st.reference ?? "", st.paidAt?.toISOString() ?? "",
