...
import { deliveryType, fulfillmentStatus, eventDetail, label, orderEventType, orderStatus, refundStatus } from "@/lib/status";
          ...
          <Panel title={t("Attribution", "유입 경로")}>
            <DetailList
              items={[
                ...
                [t("Sales link", "판매 링크"), link ? <Link key="l" href={`/seller/links/${link.id}`} className="text-[#2f4ac2] hover:underline">{link.name} ({link.code})</Link> : "—"],
              ]}
            />
          </Panel>
        </div>
