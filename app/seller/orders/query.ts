...
import { likeQ, one, periodWhere, type SP, orderProductTitleMatch } from "@/lib/server/list";
...
export function sellerOrderWhere(sellerId: string, sp: SP) {
  ...
  if (q) where.push(or(ilike(s.orders.orderNo, likeQ(q)), ilike(s.orders.buyerName, likeQ(q)), ilike(s.orders.buyerEmail, likeQ(q)), orderProductTitleMatch(likeQ(q))));
