...
export async function GET(request: Request) {
  ...
  const rows = await db
    ...
    .limit(10000);
  return csvResponse(`ringo-products-${new Date().toISOString().slice(0, 10)}.csv`, [
    ...
    ...rows.map(({ p, category }) => [p.id, p.slug, p.titleEn, p.titleKo, category, p.deliveryType, p.status, (p.priceCents / 100).toFixed(2), p.compareAtCents != null ? (p.compareAtCents / 100).toFixed(2) : "", p.currency, p.salesCount, p.rejectReason ?? "", p.createdAt.toISOString(), p.publishedAt?.toISOString() ?? ""]),
  ]);
}
