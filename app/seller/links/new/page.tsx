...
export const metadata = { title: "Create deep link" };
...
export default async function NewSellerLink({ searchParams }: { searchParams: Promise<{ productId?: string }> }) {
  ...
  return (
    <>
      <PageHeader title={t("Create deep link", "딥링크 만들기")} crumbs={[{ href: "/seller/links", label: t("Deep links", "딥링크") }, { label: t("Create", "만들기") }]} />
      {products.length === 0 ? (
