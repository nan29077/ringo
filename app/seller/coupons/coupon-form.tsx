...
import { sellerSaveCoupon } from "./actions";

type Coupon = typeof coupons.$inferSelect;

export function CouponForm({ coupon, products, currency, startsValue, endsValue }: { coupon?: Coupon; products: { id: string; titleEn: string; titleKo: string }[]; currency: string; startsValue: string; endsValue: string }) {
  ...
  return (
    <ActionForm action={sellerSaveCoupon} className="grid gap-4">
      ...
      <Panel title={t("Coupon", "쿠폰 정보")}>
        <div className="grid gap-4 md:grid-cols-2">
          <Field className="content-start" label={t("Code", "쿠폰 코드")} required hint={t("Uppercase letters, numbers, - and _. Must be unique across Ringo.", "영문 대문자, 숫자, -, _ 사용. 링고 전체에서 중복될 수 없습니다.")}>
          ...
          <Field className="content-start" label={t("Applies to", "적용 상품")}>
            <select name="productId" className="rc-select" defaultValue={coupon?.productId ?? ""}>
              <option value="">{t("All my products", "내 모든 상품")}</option>
